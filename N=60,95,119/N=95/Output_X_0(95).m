Magma V2.26-3     Fri Nov 12 2021 22:26:02 on euler    [Seed = 3744490788]
Type ? for help.  Type <Ctrl>-D to quit.
Loading "quadptssieve.m"
Loading "ozmansiksek.m"
Loading "X0N_NiceModel.m"
Dimension of CuspForms(95) is:
9
Dimesion of eigenspace lambda=1 for w19 is:
3
Dimesion of eigenspace lambda=-1 for w19 is:
6
Model for X95 is:
Curve over Rational Field defined by
x[1]^2 + 4*x[3]*x[4] + 4*x[3]*x[6] - 3*x[4]^2 + 2*x[4]*x[5] + 4*x[4]*x[6] + 
    19*x[5]^2 - 32*x[5]*x[6] + 10*x[6]^2 - x[7]^2 + 2*x[8]*x[9] + 4*x[9]^2,
x[1]*x[2] + 2*x[2]*x[6] + 3*x[3]*x[4] - 3*x[3]*x[6] - 5*x[4]^2 - 2*x[4]*x[5] + 
    6*x[4]*x[6] + 14*x[5]^2 - 26*x[5]*x[6] + 13*x[6]^2 - x[7]*x[8] + x[9]^2,
x[1]*x[3] - 2*x[3]*x[6] - 2*x[4]^2 - x[4]*x[5] + x[4]*x[6] - 2*x[5]^2 + 
    x[5]*x[6] + x[6]^2 - x[8]^2,
x[1]*x[4] - 2*x[3]*x[4] + 3*x[4]^2 - 2*x[4]*x[6] - 9*x[5]^2 + 16*x[5]*x[6] - 
    7*x[6]^2 - x[8]*x[9] - x[9]^2,
x[1]*x[5] - 2*x[3]*x[4] + x[4]^2 + 2*x[4]*x[5] - 2*x[4]*x[6] - 6*x[5]^2 + 
    10*x[5]*x[6] - 4*x[6]^2 - x[9]^2,
x[1]*x[6] - x[3]*x[4] - x[3]*x[6] - x[4]^2 + x[4]*x[5] + x[4]*x[6] - x[5]^2 - 
    x[5]*x[6] + 2*x[6]^2,
x[1]*x[8] - x[2]*x[7] + x[5]*x[7] + 2*x[5]*x[8] - x[5]*x[9] - x[6]*x[7] + 
    x[6]*x[9],
x[1]*x[9] - x[3]*x[7] + x[4]*x[7] - x[5]*x[8] + 3*x[5]*x[9] + x[6]*x[7] + 
    x[6]*x[8] - x[6]*x[9],
x[2]^2 - 4*x[5]^2 + 8*x[5]*x[6] - 4*x[6]^2 - x[8]^2,
x[2]*x[3] - 2*x[2]*x[6] - 2*x[3]*x[4] + 2*x[3]*x[6] + 4*x[4]^2 - 2*x[4]*x[5] - 
    2*x[4]*x[6] - 8*x[5]^2 + 18*x[5]*x[6] - 10*x[6]^2 - x[8]*x[9] - x[9]^2,
x[2]*x[4] - x[2]*x[6] - 2*x[3]*x[4] + 2*x[3]*x[6] + 2*x[4]^2 + 2*x[4]*x[5] - 
    2*x[4]*x[6] - 8*x[5]^2 + 14*x[5]*x[6] - 8*x[6]^2 - x[9]^2,
x[2]*x[5] - x[2]*x[6] - x[3]*x[4] + x[3]*x[6] + x[4]^2 - 2*x[5]^2 + 4*x[5]*x[6] 
    - 3*x[6]^2,
x[2]*x[8] - x[3]*x[7] - x[5]*x[8] + 2*x[6]*x[7] + x[6]*x[8],
x[2]*x[9] - x[4]*x[7] + x[5]*x[7] - x[5]*x[9] + x[6]*x[9],
x[3]^2 - 2*x[3]*x[4] - 2*x[3]*x[6] + x[4]^2 + 4*x[4]*x[5] - 2*x[4]*x[6] - 
    8*x[5]^2 + 12*x[5]*x[6] - 3*x[6]^2 - x[9]^2,
x[3]*x[5] - x[3]*x[6] - x[4]^2 + x[4]*x[5] + x[4]*x[6] - 3*x[5]*x[6] + 2*x[6]^2,
x[3]*x[8] - x[4]*x[7] - x[5]*x[8] + x[6]*x[7] - x[6]*x[8],
x[3]*x[9] - x[5]*x[7] - x[5]*x[9] + x[6]*x[7] - x[6]*x[9],
x[4]*x[8] - x[5]*x[7] - x[5]*x[8] - x[5]*x[9] + x[6]*x[7] + x[6]*x[9],
x[4]*x[9] - x[5]*x[8] + x[6]*x[8] - x[6]*x[9],
x[7]*x[9] - x[8]^2 + x[8]*x[9] + x[9]^2

w19 on X0(95) is given by:
Mapping from: Crv: X95 to Crv: X95
with equations : 
u[1]
u[2]
u[3]
u[4]
u[5]
u[6]
-u[7]
-u[8]
-u[9]
and inverse
u[1]
u[2]
u[3]
u[4]
u[5]
u[6]
-u[7]
-u[8]
-u[9]

Genus of X0(95) is  9
Genus of X0(95)/w19 is  3

By finding poles of j-map, we find that we have these 4 cusps:
[
    Place at (-3/5 : 0 : -2/5 : -1/5 : -1/5 : -1/5 : 1 : 0 : 0),
    Place at (3/5 : 0 : 2/5 : 1/5 : 1/5 : 1/5 : 1 : 0 : 0),
    Place at (-1 : 0 : 0 : 0 : 0 : 0 : 1 : 0 : 0),
    Place at (1 : 0 : 0 : 0 : 0 : 0 : 1 : 0 : 0)
]

One quadratic but not rational point is P5: (1/14*(-r19 - 17) : 1/7*(r19 - 11) :
1/7*(r19 + 3) : 1/7*(r19 - 4) : 1/14*(r19 + 3) : 1 : 0 : 0 : 0)
Is P5 fixed by w19?
true
Hence, P5 can't come from a pullback of rational point.

Is D1:=Divisor(P1) - Divisor(P3) of order 180?
true

Is D2:=Divisor(P1)-Divisor(P2)-10*D1 of order 6?
true

Is D2:=Divisor(P1)-Divisor(P2)-10*D1 of order 6 in J(Q)_tor/<D1>?
true
Hence, D2 and D1 generate a subgroup isomorphic to Z/6Z + Z/180Z

J(Q)_tor is isomorphic to a subgroup of:
Abelian Group isomorphic to Z/2 + Z/2 + Z/60 + Z/180
Defined on 4 generators in supergroup CG:
    JF3.1 = CG.1
    JF3.2 = CG.2
    JF3.3 = CG.3
    JF3.4 = CG.4
Relations:
    2*JF3.1 = 0
    2*JF3.2 = 0
    60*JF3.3 = 0
    180*JF3.4 = 0

J(Q)_tor is isomorphic to a subgroup of:
Abelian Group isomorphic to Z/2 + Z/2 + Z/2 + Z/2 + Z/2 + Z/18 + Z/90 + Z/900
Defined on 8 generators in supergroup CG:
    JF7.1 = CG.1
    JF7.2 = CG.2
    JF7.3 = CG.3
    JF7.4 = CG.4
    JF7.5 = CG.5
    JF7.6 = CG.6
    JF7.7 = CG.7
    JF7.8 = CG.8
Relations:
    2*JF7.1 = 0
    2*JF7.2 = 0
    2*JF7.3 = 0
    2*JF7.4 = 0
    2*JF7.5 = 0
    18*JF7.6 = 0
    90*JF7.7 = 0
    900*JF7.8 = 0
Hence, J(Q)_tor doesn't have two independent elements of order 4.

J(Q)_tor is isomorphic to a subgroup of:
Abelian Group isomorphic to Z/4 + Z/12 + Z/36 + Z/1658340
Defined on 4 generators in supergroup CG:
    JF11.1 = CG.1
    JF11.2 = CG.2
    JF11.3 = CG.3
    JF11.4 = CG.4
Relations:
    4*JF11.1 = 0
    12*JF11.2 = 0
    36*JF11.3 = 0
    1658340*JF11.4 = 0
Hence, J(Q)_tor doesn't have two independent elements of order 10.

Since we have found a torsion subgroup iso. to Z/6Z + Z/180Z, and rk(J0(95)(Q)) 
== 0,
we know that J(Q) is a subgroup of:
Z/2Z + Z/2Z + Z/6Z + Z/180Z

We have found  11  points on X_0(95)^2(Q).
2 of them are pullbacks of rationals from X0(95)/w19.
9 of them are non-pullbacks
Succeeded in proving that we have all exceptional quadratic pts? (true if 
succeeded, number otherwise)
11
13
13
3
11
true

Hence, there are no quadratic points on X0(95) not coming from X0(95)/w19(Q).
We now have to find rational points on X0(95)/w19(Q) and check their pullbacks.

X0(95)/w19 is actually  Hyperelliptic Curve defined by y^2 = x^8 - 2*x^7 - 7*x^6
    + 16*x^5 - 2*x^4 - 2*x^3 - 4*x^2 + 5 over Rational Field
Rank of J(X0(95)/w19)(Q) is 0
Hence rk(J(X0(95)/w19)(Q)) = rk(J0(95)(Q)) = 0.

J(X0(95)/w19)(Q)_tors has at most  20  points.
We have found  20  points on J(X0(95)/w19)(Q) out of possible  20
Hence, we know whole J(X0(95)/w19)(Q).

Using Mumford representations, we get that x coordinates of all rational
plus non-obvious quadratic points on X0(95)/w19 are (excluding pts at infinity):
1/2*(w + 1)
1/2*(-w + 1)
1/2*(-w + 3)
1/2*(w + 3)
Hence, only rational points are points at infinity

Pullback of point  (1 : -1 : 0)  is:
[
    <Place at (-3/5 : 0 : -2/5 : -1/5 : -1/5 : -1/5 : 1 : 0 : 0), 1>,
    <Place at (3/5 : 0 : 2/5 : 1/5 : 1/5 : 1/5 : 1 : 0 : 0), 1>
]
Pullback of point  (1 : 1 : 0)  is:
[
    <Place at (-1 : 0 : 0 : 0 : 0 : 0 : 1 : 0 : 0), 1>,
    <Place at (1 : 0 : 0 : 0 : 0 : 0 : 1 : 0 : 0), 1>
]
Hence, the only quadratic point up to Galois conjugacy, apart from rational 
cusps, is  (1/14*(-r19 - 17) : 1/7*(r19 - 11) : 1/7*(r19 + 3) : 1/7*(r19 - 4) : 
1/14*(r19 + 3) : 1 : 0 : 0 : 0)

Total time: 703.000 seconds, Total memory usage: 233.97MB
